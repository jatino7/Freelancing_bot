...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            // UPDATED: Now goes to OnboardingActivity first to pick a role
    ...
    private fun getUserData() {
        ...
        dbRef.child(uid).get().addOnSuccessListener { snapshot ->
            if (snapshot.exists()) {
                // Ensure we retrieve the role correctly from the Firebase Snapshot
                val role = snapshot.child("role").getValue(Int::class.java) ?: 1

                Log.d("LoginRole", "User role retrieved: $role")

                // Save to SharedPrefs so the app knows the role throughout the session
            ...
            binding.pgBar.visibility = View.GONE
            Functions.showAlert(e.localizedMessage ?: "Database error occurred", this)
        }
    }
}